...
contract SymmStaking is Initializable, AccessControlEnumerableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
	...
	// State Variables
	//--------------------------------------------------------------------------
	address public stakingToken; //symm token

	uint256 public totalSupply; //total symm staked
	...
	function lastTimeRewardApplicable(address _rewardsToken) public view returns (uint256) {
		return block.timestamp < rewardState[_rewardsToken].periodFinish ?
		 block.timestamp : rewardState[_rewardsToken].periodFinish;
	}
	...
	function _addRewardsForToken(address token, uint256 amount) internal {
		...
		if (block.timestamp >= state.periodFinish) {
			...
			uint256 leftover = remaining * state.rate; // reward tokens to be generated from period.finish-block.timestamp
